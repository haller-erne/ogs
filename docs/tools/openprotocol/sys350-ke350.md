...
# KE350 MultiSpindle tightening system
...
This is the preferred configuration, as the user-experience is most intuitive. Unfortunately the most often used start grip handles do not provide the neccessary signals, as a start grip handle with three outputs is needed:
...
This forces the operator to switch the CW/CCW selection switch to CCW every time a loosen process is needed. Please note, that 
no custom loosen program number can be selected, as the CCW start 
always forces program 99 on the connected channels. However, this
automatically selects the correct spindles (those defined in the 
fastening operation), so this is basically a good thing.

#### No CCWSel, but NokAck

As most start handles only provide start CW and start CCW signals (without a signal for indicating the CW/CCW position), CCWSel often cannot be used. 

To ensure sequence interlock between tightening and loosening, 
the `FO 1 NokAck` signal is connected to an external pushbutton (typically the start handle provides this).
