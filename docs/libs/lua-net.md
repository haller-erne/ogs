...
| `put(url: string, body: string = nil, auth_bearer_token: string = nil)` | `status: number, data: string, statustext: string` | Send a put request to the given endpoint `URL`, with put `data` in the request body, optinally adding the `auth_bearer_token` value to the `Authorization` header.  | None |
...
| `put_async(url: string, body: string = nil, auth_bearer_token: string = nil)` | `handle: number` or `nil` | Starts an async put request (see the blocking `put` above for parameter descriptions). On success a `handle` is returned, which can be used for polling the request status or aborting the request.  | None |
